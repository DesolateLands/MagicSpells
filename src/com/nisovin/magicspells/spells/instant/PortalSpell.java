...
public class PortalSpell extends InstantSpell {
	...
	class PortalLink implements Listener {
		...
		void startTasks() {
			if (PortalSpell.this.effectInterval > 0) {
				this.taskId1 = MagicSpells.scheduleRepeatingTask(new Runnable() {
					...
					public void run() {
						if (caster.isValid()) {
							playSpellEffects(EffectPosition.SPECIAL, loc1, caster);
							playSpellEffects(EffectPosition.SPECIAL, loc2, caster);
						} else {
							disable();
						}
					}
			...
			if (this.caster.isValid()) {
				...
				if (this.box1.contains(event.getTo())) {
					if (checkTeleport(player)) {
						...
						event.setTo(loc);
						playSpellEffects(EffectPosition.TARGET, player, this.caster);
					}
		...
		void disable() {
			playSpellEffects(EffectPosition.DELAYED, this.loc1, this.caster);
			playSpellEffects(EffectPosition.DELAYED, this.loc2, this.caster);
